...
pub const TilemapEditor = struct {
    ...
    fn drawTile(self: @This(), tl: imgui.ImVec2, tile: usize, zoom: usize) void {
        ...
        const uv1 = imgui.ImVec2{ .x = rect.x + rect.width, .y = rect.y + rect.height };

        imgui.ogImDrawList_AddImage(imgui.igGetWindowDrawList(), self.tileset.tex.imTextureID(), tl, br, uv0, uv1, 0xffffffff);
    }
};
