...
pub fn ogOpenPopup(str_id: [*c]const u8) void {
    imgui.igOpenPopup(str_id);
}

pub fn ogColoredButton(color: imgui.ImU32, label: [*c]const u8) bool {
    return ogColoredButtonEx(color, label, .{});
}

pub fn ogColoredButtonEx(color: imgui.ImU32, label: [*c]const u8, size: imgui.ImVec2) bool {
...
pub fn ogGetWindowCenter() imgui.ImVec2 {
    var max = imgui.ogGetWindowContentRegionMax();
    max.x /= 2;
    max.y /= 2;
    return max;
}
...
pub fn ogAddQuadFilled(draw_list: [*c]imgui.ImDrawList, tl: imgui.ImVec2, size: f32, col: imgui.ImU32) void {
    ogImDrawList_AddQuadFilled(draw_list, &imgui.mVec2{ .x = tl.x, .y = tl.y }, &imgui.ImVec2{ .x = tl.x + size, .y = tl.y }, &imgui.ImVec2{ .x = tl.x + size, .y = tl.y + size }, &imgui.ImVec2{ .x = tl.x, .y = tl.y + size }, col);
}
...
pub fn ogInputTextEnter(label: [*c]const u8, buf: [*c]u8, buf_size: usize) bool {
    return imgui.igInputText(label, buf, buf_size, imgui.ImGuiInputTextFlags_EnterReturnsTrue | imgui.ImGuiInputTextFlags_AutoSelectAll, null, null);
}
